...
const MovieCard: React.FC<MovieCardProps> = ({
  ...
  const browseType = data.type === 'show' ? 'show' : 'movie';
  const displayName = localizeName(locale, data, data.nameEn);

  const setZIndexDebounce = React.useMemo(() => debounce(setZIndex, 500), []);

  return (
    <Link
      href={`/browse/${browseType}/${data.id}`}
      aria-label={t('movieCard.openDetailsAria', { title: displayName })}
    >
      <div
                ...
                {!!data.childrenCount && (
                  <span>
                    ...
                    {data.childrenCount === 1
                      ? t('movieCard.seasonSingular')
                      : t('movieCard.seasonPlural')}{' '}
                  </span>
                )}
              ...
              <div className='evidenceList overflow-hidden overflow-ellipsis'>
                {data.genres.length &&
                  data.genres.map((genre) => (
                    <div
                      className='evidenceItem content-dot inline-block'
                      key={genre.nameEn}
                    >
